        ...
        float aimAngle, out float adjAngle)
    {
        ...
        if (!chopping)
        {
            if (IsLolly(pawn))
            {
                ...
                {
                    return compBigChoppa.Props.northOffset;
                }

                if (rotation == Rot4.East)
                {
                    return compBigChoppa.Props.eastOffset;
                }

                if (rotation == Rot4.South)
                {
                    return compBigChoppa.Props.southOffset;
                }

                return rotation == Rot4.West ? compBigChoppa.Props.westOffset : result;
            }
